...
- 以后在本项目中完成一轮代码实现后，默认要做一次 Git 版本留档，除非用户在本轮明确要求不要提交或不要推送。
- 标准顺序固定为：先做与改动范围匹配的验证，再做本地 `commit`，最后推送到远端 `origin`。
- 如果远端 `main` 无法直接推送，默认改为推送到远端备份分支；分支名优先使用 `codex/` 前缀，并包含日期或本轮主题，确保每次代码改动都有远端备份。
- 每次完成代码改动后的汇报中，必须明确写出本轮版本信息，至少包括：本地 commit 哈希、远端备份分支名（或实际推送目标）、验证结果。
- 每次完成代码改动后的汇报中，必须明确写出本轮改动内容摘要，至少包括：改了哪些文件、修了什么问题、还剩哪些未完成项或风险。
- 不要把明显未完成、未验证或已知失败的中间状态提交或推送出去。
- 如果远端推送失败，要明确告知失败原因，并保留已完成的本地 commit；必要时提醒用户检查网络、权限或远端仓库状态。
