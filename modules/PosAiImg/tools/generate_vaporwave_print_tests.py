...
OUTPUT_DIR = ROOT / "印花图_透明底" / "蒸汽波印花_5张测试_2026-06-14"
SERVER = "http://127.0.0.1:8188"
CHECKPOINT = "Juggernaut-XL_v9_RunDiffusionPhoto_v2.safetensors"

BASE_POSITIVE = (
    "standalone isolated screen print decal asset, vaporwave t-shirt print badge, retro futurism, "
    "neon magenta, cyan, purple, deep navy, white highlights, chrome gradient accents, halftone screen print texture, "
    "compact central composition with clean outer contour, visible on both black and white t-shirts, "
    "centered isolated artwork on seamless plain pure white background, generous empty margin, "
    "single print asset only, no mockup, no model, no shirt, no clothing, no background scene, no shadow, "
    "no readable text, no letters, no logo, no brand mark"
)

NEGATIVE = (
    "person, human, face, portrait, statue head, bust, skull, skeleton, body, hands, model, mannequin, "
    "shirt mockup, t-shirt silhouette, collar, sleeves, fabric folds, clothing, apparel, product preview, "
    "readable text, letters, words, logo, brand name, watermark, signature, messy full background, "
    "wallpaper, poster scene, photorealistic city photo, low contrast, blurry, cropped"
)
...
PROMPTS = [
    (
        "01_neon_sunset_grid",
        "霓虹日落网格",
        "neon sunset grid badge, striped sun disc, perspective grid floor, small palm silhouettes, chrome ring frame, compact t-shirt emblem",
    ),
    (
        "02_palm_window",
        "棕榈复古窗口",
        "retro computer window badge, palm tree silhouettes, pixel moon, cyan magenta frame, vaporwave screen print decal, no text",
    ),
    (
        "03_chrome_geometry",
        "铬感几何",
        "chrome geometric orb badge, wireframe triangle, neon grid lines, magenta cyan glow accents, compact sticker emblem",
    ),
    (
        "04_ocean_synthwave",
        "合成海浪",
        "synthwave ocean badge, stylized wave, striped sun, horizon grid, palm leaves, bold screen print composition",
    ),
    (
        "05_cassette_neon",
        "霓虹磁带",
        "retro cassette tape vaporwave badge, neon grid halo, chrome tape reels, magenta cyan accents, no readable label",
    ),
]
...
def make_workflow(prompt: str, seed: int) -> dict:
    ...
    return {
        ...
        "9": {"class_type": "SaveImage", "inputs": {"filename_prefix": "vaporwave_print_test", "images": ["8", 0]}},
    }
...
def add_light_edge(img: Image.Image) -> Image.Image:
    ...
    edge_arr = np.clip(edge_arr, 0, 90).astype(np.uint8)
    edge = Image.new("RGBA", rgba.size, (245, 245, 255, 0))
...
def main() -> int:
    ...
    for index, (slug, title, prompt) in enumerate(PROMPTS, start=1):
        prompt_id = http_json("POST", f"{SERVER}/prompt", {"prompt": make_workflow(prompt, 2026061400 + index * 119)}, timeout=30)["prompt_id"]
