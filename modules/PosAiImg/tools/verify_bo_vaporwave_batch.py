...
COUNT = 50
END = START + COUNT - 1
BATCH = f"蒸汽波印花_BO-{START}-BO-{END}_2026-06-14"
...
def alpha_ok(paths: list[Path]) -> bool:
    for path in paths:
        img = Image.open(path).convert("RGBA")
        alpha_min, alpha_max = img.getchannel("A").getextrema()
        if alpha_min != 0 or alpha_max == 0:
            print(f"bad alpha: {path.name} extrema={(alpha_min, alpha_max)}")
            return False
    return True
...
def main() -> int:
    ...
    require(alpha_ok(print_files), "transparent PNG alpha check failed")
    ...
    print(f"overview={MOCKUP_DIR / '_overview.jpg'}")
    return 0
