...
COUNT = 50
END = START + COUNT - 1
BATCH = f"高街黑白灰金属素描印花_BO-{START}-BO-{END}_2026-06-13"
...
def xlsx_rows(path: Path) -> list[list[str]]:
    ...
    for row in root.findall(".//main:sheetData/main:row", ns):
        rows.append([inline_text(cell) for cell in row.findall("main:c", ns)])
    return rows
...
def main() -> int:
    ...
    require(len(print_files) == COUNT, f"print count {len(print_files)} != {COUNT}")
    require(len(mockup_files) == COUNT, f"mockup count {len(mockup_files)} != {COUNT}")
    require(len(putaway_files) == COUNT, f"putaway count {len(putaway_files)} != {COUNT}")
    ...
    require(len(rows) == COUNT + 1, f"xlsx rows {len(rows)} != {COUNT + 1}")
